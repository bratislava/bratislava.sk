...
function addRelation(model, attribute, type, modelF = undefined, attributeF = undefined) {
  relations.push({
    model,
    attribute,
    type,
...
function processRelation(key, value, collectionName) {
  if (value.model) {
    ...
    if (value.column) {
      ...
      addRelation(collectionName, key, 'oneToMany', value.collection, value.via)
    }
  }
}
...
async function migrateOneToOneRelation(relation) {
  if (pluralize(relation.model, 1) === relation.modelF) {
    await migrate(relation.model, relation.table, (item) => oneToOneCirvleRelationMapper(relation, item))
  } else {
...
async function migrateManyToManyRelation(relation, sourceTable) {
  if (pluralize(relation.model, 1) === relation.modelF) {
    ...
    await migrate(sourceTable, relation.table, ({ id, ...item }) => migrateItem(item))
  }
}
...
async function migrateRelations(tables) {
  ...
  if (isPGSQL) {
    v4Tables = (await dbV4('information_schema.tables').select('table_name').where('table_schema', 'public')).map(
      (row) => row.table_name
    )
  }
  ...
  if (isMYSQL) {
    v4Tables = (await dbV3('information_schema.tables').select('table_name')).map((row) => row.table_name)
  }
    ...
    if (relation.type === 'oneToOne') {
      ...
      var sourceTable = v3RelationTables.find(
        (t) =>
          ...
          (t.startsWith(`${relation.modelF}`) && t.endsWith(`__${relation.model}_${relation.attribute}`))
      )
...
async function migrateModels(tables) {
  ...
  const modelsDefs = await dbV3('core_store').where('key', 'like', 'model_def_application::%')
  ...
  for (const modelDefEntry of modelsDefs) {
    ...
    await migrate(modelDef.collectionName, modelDef.collectionName, (item) => {
      if (modelDef.options.timestamps === false) {
        ...
        const timestamps =
          modelDef.options.timestamps === true ? ['created_at', 'updated_at'] : modelDef.options.timestamps
        ...
        return migrateItem(omit(newItem, [...omitAttributes, createdAt, updatedAt]))
      }
    })
  }
