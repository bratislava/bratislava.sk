...
const processedTables = ['users-permissions_role', 'users-permissions_permission', 'users-permissions_user']
...
async function migrateUserPermissions() {
  ...
  const count =
    (await sourceSelect.clone().count().first()).count || (await sourceSelect.clone().count().first())['count(*)']
  const countTotal = (await dbV3(source).count().first()).count || (await dbV3(source).count().first())['count(*)']
  ...
  for (var page = 0; page * BATCH_SIZE < count; page++) {
    ...
    const migratedItems = migrateItems(items, ({ type, controller, action, enabled, policy, role, ...item }) => ({
      ...migrateItem(item),
...
async function migrateUsersData() {
  ...
  const count = (await dbV3(source).count().first()).count || (await dbV3(source).clone().count().first())['count(*)']
