...
const { addAdditionalColumns } = require('./addRequiredColumns')
...
async function migrate() {
  ...
  if (isPGSQL) {
    tables = (await dbV3('information_schema.tables').select('table_name').where('table_schema', 'public')).map(
      (row) => row.table_name
    )
  }
  ...
  if (isMYSQL) {
    tables = (await dbV3('information_schema.tables').select('table_name')).map((row) => row.table_name)
  }
