...
const Button = forwardRef<HTMLAnchorElement | HTMLButtonElement, PolymorphicProps>(
  ({ className, disabled = false, variant = 'brand', size = 'lg', icon, text, startIcon, endIcon, ...rest }, ref) => {
    ...
    const style = cx(
      ...
      {
        // text for lg button
        'px-6 py-4 text-lbl-1 leading-5':
          size === 'lg' &&
          !icon &&
          text &&
          (variant === 'brand' ||
        ...
        // text for sm button
        'px-5 py-3 text-lbl-2 leading-5':
          size === 'sm' &&
          !icon &&
          text &&
          (variant === 'brand' ||
        ...
        'px-2.5 py-2.5':
          ...
          (variant === 'brand' ||
            ...
            variant === 'black-outline'),
        // icon for lg button
        'px-2.5 py-2.5 text-lbl-1 leading-6':
          size === 'lg' && icon && !text && (variant === 'plain-brand' || variant === 'plain-black'),
        // icon for sm button
        'px-2 py-2 text-lbl-2': size === 'sm' && icon && !text && (variant === 'plain-brand' || variant === 'plain-black'),

        // text for lg button
        'px-3 py-2 text-lbl-1 leading-6':
          size === 'lg' &&
          !icon &&
          text &&
          (variant === 'plain-brand' || variant === 'plain-black' || variant === 'plain-negative'),
        // text for sm button
        'px-2 py-1 text-lbl-2 leading-6':
          size === 'sm' &&
          !icon &&
          text &&
          (variant === 'plain-brand' || variant === 'plain-black' || variant === 'plain-negative'),
        ...
        'text-lbl-1 leading-8': size === 'lg' && (variant === 'link-brand' || variant === 'link-black'),
        // text for sm link button
        'text-lbl-2 leading-6': size === 'sm' && (variant === 'link-brand' || variant === 'link-black'),
