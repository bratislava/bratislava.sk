...
const SingleCheckBox = ({ error = false, children, isDisabled = false, variant = 'basic', ...rest }: CheckBoxBase) => {
  ...
  const checkboxStyle = cx(
    'relative focus-visible: outline-none focus:outline-none appearance-none items-center justify-center w-6 h-6 rounded border-2 border-solid border-gray-universal-700 left-0 right-0 top-0 bottom-0',
    {
  ...
  const containerStyle = cx('group flex flex-row absolute items-center justify-center p-0 gap-4', rest.className, {
  ...
  const labelStyle = cx(
    'select-none not-italic font-normal text-default leading-8 text-gray-universal-700 flex-none order-1 grow',
    {}
  )
