...
export const DateSelectShortcuts = ({
  value,
  onClick,
}: DateSelectShortcutsProps) => {
    ...
    <>
      {new Array(5).fill(1).map((_, i) => {
        ...
        return (
          <button
            className={cx(
              'bg-input-nav-bg px-2 sm:px-4 relative cursor-pointer',
              {
          ...
          >
            {title}
            {isActive && (
              <div className="absolute top-1/2 left-1/2 bg-current w-1 h-1 rounded-full mt-4" />
            )}
          </button>
        );
      })}
