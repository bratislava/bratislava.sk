  ...
  return (
    <div className={cx(className, 'flex flex-col md:flex-row text-font gap-3')}>
      <div className="w-full md:w-1/2 mb-6 md:mb-0">
        <div
          className={cx('flex flex-col h-full', { 'justify-center': !address })}
        >
          {title && (
            <h4 className="mb-6 font-semibold text-md leading-[36px]">
              {title}
            </h4>
          )}
          {description && (
    ...
    <div className="flex flex-col items-center justify-center h-full text-default leading-[30px]">
      ...
      {label && href && (
        <a
          href={href}
          className={cx(
            'mt-8 px-6 py-3 font-medium border-2 rounded-lg shadow-sm whitespace-nowrap',
            {
