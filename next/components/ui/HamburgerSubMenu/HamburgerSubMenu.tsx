  ...
  return (
    ...
    >
        ...
        >
          {variant === 'homepage' ? (
            <div className="flex items-center">
              ...
              <span className="text-left text-p2 font-semibold">{item.title}</span>
            </div>
          ) : (
            ...
            return (
              <React.Fragment key={subItem.title}>
                <div className="flex items-center gap-x-5">
                  ...
                  <p className="text-p1 font-semibold">{subItem.title}</p>
                </div>
                  ...
                  {subItem.subItems.length > 2 && (
                    <UnstyledLink href={subItem.url}>
                      ...
                      >
                        {isExpanded ? (
                          <>
                            <p className="text-p2 font-semibold underline">{t('showLess')}</p>
                            <ChevronUp />
                          </>
                        ) : (
                          <>
                            <p className="text-p2 font-semibold underline">{t('showMore')}</p>
                            {/* <ChevronDown /> */}
                            <ChevronRight />
                          </>
                        )}
      ...
      >
        ...
        <div className="mt-4 text-center text-p2 font-semibold text-font">{t('closeMenu')}</div>
      </div>
    </div>
  )
}
