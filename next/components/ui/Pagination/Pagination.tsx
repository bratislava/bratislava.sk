...
export const ThreeDots = () => {
  return (
    <div className="ml-2 w-7 sm:ml-3 sm:w-8">
      <span className="text-default font-semibold text-gray-500">&hellip;</span>
    </div>
  )
}
      ...
      <div className="m-auto flex w-auto items-center justify-between">
        ...
        {items.map((item, index) =>
          item.page < numberOfPages && item.page > 1 ? (
            ...
            >
              <div className="text-gray-700/50 text-p1 m-auto block font-semibold leading-5">{item.page}</div>
            </Button>
          ) : null
        )}
