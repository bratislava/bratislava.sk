        ...
        return (
          <div key={index} className={cx({ 'mt-5': index > 0 })}>
            ...
            >
                ...
                return (
                  <div className="space-y-6" key={row}>
                    {row == 0 && fileSection.category && !hideCategory && (
                      <span className="text-default md:text-md font-medium">
                    ...
                    <div
                      className={cx(
                        'grid grid-cols-1 w-full gap-y-6',
                        ...
                        .map((file, index) => (
                          <div key={index} className="w-full">
                            <DownloadCard
                              ...
                              downloadLink={
                                file.media?.url ? file.media?.url : ''
                              }
                              uploadDate={
                                file.media?.created_at
                                  ? file.media?.created_at
                                  : ''
                              }
                              downloadDetail={
              ...
              {length > 6 && (
                <Button
                  className="self-center text-default px-6 py-2.5"
                  variant="transparent"
                  onClick={handleClick}
                >
                  {buttonText}
                ...
                {fileSection?.files.map((file, index) => (
                  <div key={index}>
                    <DownloadCard
                      ...
                      uploadDate={
                        file.media?.created_at ? file.media?.created_at : ''
                      }
                      downloadDetail={
                        file.media?.ext && file.media.size
                          ? `${file.media?.ext?.toUpperCase()}; ${file.media?.size.toString()} kB`
