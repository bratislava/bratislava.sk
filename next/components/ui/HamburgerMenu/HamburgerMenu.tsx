    ...
    >
      <div className={cx('flex-1 flex flex-col bg-secondary px-8 pb-11', className)}>
          ...
          {hamburgerMenuItems.map((item) => {
            return (
              ...
              >
                ...
                <p className="w-36 text-p2 font-medium">{item.title}</p>
                <ChevronRight />
              </button>
            )
          })}
        ...
        <div className="hidden items-center justify-between py-8 lg:flex">
          <Link className="text-p2 font-medium" variant="plain" href="#">
            {t('login')}
          </Link>
        </div>
          ...
          {MOCK_HAMBURGER_MENU_ITEMS[lang].map((item, index) => (
            ...
            >
              <button className="flex items-center gap-x-3" onClick={() => closeMenu()} type="button">
                {item?.icon}
                <span className="text-p2 font-medium">{t(item.title)}</span>
              </button>
            </Link>
          ))}
        </div>
