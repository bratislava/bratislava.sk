...
import CarouselControl from '@components/organisms/Carousel/CarouselControl'
import cx from 'classnames'
import { useTranslations } from 'next-intl'
import React, { ReactNode, useRef, useState } from 'react'
import { twMerge } from 'tailwind-merge'

// Inspired by mestskakniznica.sk project
...
const Carousel = ({
  ...
  const shiftCount = shiftIndex === 'single' ? 1 : visibleCount
  ...
  return (
    <div className={twMerge('relative', className)}>
      {useOldStyledControls && (
        ...
        >
          <CarouselControl direction="left" onPress={handleGoToPrevious} />
        </div>
      )}
      ...
      {useOldStyledControls && (
        ...
        >
          <CarouselControl direction="right" onPress={handleGoToNext} />
        </div>
      )}
