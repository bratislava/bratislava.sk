...
const CyclingTotalChart = ({
  className,
  location,
  direction,
  ...rest
}: IProps) => {
  const [cyclingData, setCyclingData] =
    React.useState<FetchOpenDataResult['data']>();
  ...
  const totalSeries: ISeries[] = filteredData.map((dataByYear) => {
    const year = dataByYear.name.substring(
      dataByYear.name.lastIndexOf('-') + 1
    );

    return {
      name: year,
      data: Array.from(Array(MONTHS)).map((_, ix) => [
        ix + 1,
