...
const GeneralPage = ({
  ...
  return (
    <BasePageLayout
      footer={footer}
      menuItems={menuItems}
      activeMenuItem={page.pageCategory?.id}
    >
      ...
      >
        ...
        <SectionContainer>
          <div className="min-h-[220px] relative">
            ...
            <h1 className="pt-30 text-md md:text-2xl font-bold whitespace-pre-wrap mb-10">
              {page?.title}
            </h1>
            ...
            {page?.pageButtonContent &&
              (page?.pageButtonContent.title ||
                ...
                >
                  <UILink
                    href={parsePageLink(page.pageButtonContent)?.url ?? ''}
                  >
                    <span>
                      {parsePageLink(page.pageButtonContent)?.title ?? ''}
                    </span>
                  </UILink>
                </Button>
              )}
                ...
                switch (section.__typename) {
                  case 'ComponentSectionsSubpageList':
                    return (
                      <SubpageList
                        ...
                        className="mt-10"
                        key={index}
                        subpageList={section.subpageList
                          ?.map(parsePageLink)
                          .filter(isPresent)}
                      />
                    );
                  case 'ComponentSectionsFeaturedBlogPosts':
                    ...
                    return (
                      <div
                        key={index}
                        className="z-10 w-full absolute -bottom-40 lg:-bottom-87 overflow-hidden"
                      >
