...
const SelectFieldShowCase = () => {
  ...
  return (
    <Wrapper direction="column" title="SelectField">
      <Stack>
        <SelectField label="Select Field" enumOptions={enumOptions} type="one"
                     value={selectValueFirst} onChange={value => setSelectValueFirst(value)}/>
        <SelectField label="Select Field" enumOptions={enumOptions} dropdownDivider placeholder="Test placeholder" type="one"
                     value={selectValueSecond} onChange={value => setSelectValueSecond(value)}/>
        <SelectField label="Select Field" enumOptions={enumOptions} disabled type="one" onChange={value => console.log(value)}/>
      </Stack>
      <Stack>
        <SelectField label="Select Field" enumOptions={enumOptions} dropdownDivider
                     value={selectValueThird} onChange={value => setSelectValueThird(value)} type="multiple"/>
        <SelectField label="Select Field" enumOptions={enumOptions} selectAllOption placeholder="Multi-choice"
      ...
      <Stack>
        ...
        <SelectField label="Select Field" enumOptions={enumOptions} placeholder="Radio"
                     value={selectValue6} onChange={value => setSelectValue6(value)} type="radio"/>
      </Stack>
      <Stack direction="column">
        <SelectField label="Select Field"  errorMessage="Test error message" type="one" onChange={value => console.log(value)}/>
        <SelectField label="Select Field"  description="This is simple description" type="one" onChange={value => console.log(value)}/>
      </Stack>
      <Stack>
        <SelectField label="Select Field" tooltip="This is example of tooltip" type="one" onChange={value => console.log(value)}/>
        <SelectField label="Select Field" tooltip="This is another example of tooltip" type="one" required onChange={value => console.log(value)}/>
      </Stack>
    </Wrapper>
  )
}
