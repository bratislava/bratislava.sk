...
const LoadingOverlay = ({ loading, children }: PropsWithChildren<LoadingOverlayProps>) => {
  return (
    ...
    <div className={cn('relative', { 'min-h-[160]': loading })}>
      {loading ? (
        <div className="absolute top-0 left-0 flex size-full flex-col items-center text-content-passive-inverted-primary/80">
          <ShrinkablePadding />
          {/* For some reason it displays behind aspect-w-* elements, so z-index is added. */}
          <LoadingSpinner className="z-10 shrink-0 text-content-passive-inverted-primary" />
          <ShrinkablePadding />
        </div>
      ) : null}
      {children}
