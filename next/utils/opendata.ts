...
class OpenDataClient {
  ...
  private _getData = async ({
    ...
    try {
      ...
      if (datasetFiles.length === 0)
        throw new Error(`Data set with id ${dataset.id} has no files`);

      // Then download all JSON files from this dataset
      const resultData = await this._downloadAllFiles(
        datasetFiles,
        resultFileType
      );
  ...
  private _getCacheKey = ({ type, id, action }: IFetchOptions) =>
    `${type}@${id}@${action}`;

  private _handleFetch = async <T>({
    type,
    id,
    action,
