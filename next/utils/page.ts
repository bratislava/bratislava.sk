...
const getColorVariables = (pageColor: string) =>
  COLOR_VARIABLES[pageColor] ?? COLOR_VARIABLES.red; // use red as default
...
export const formatFiles = (files: FileFragment[]): TFile[] =>
  files.map((file) => ({
    ...
    media: {
      ...
      created_at: file.media?.created_at
        ? getLocalDate(file.media?.created_at)
        : '',
...
export const parseRelatedBlogPosts = (
  ...
  RelatedContentBlogPosts.map((relatedBlogPost) => ({
    ...
    updated_at: getNumericLocalDate(relatedBlogPost.created_at)
      .split('.')
      .join('.  '),
    created_at: getNumericLocalDate(relatedBlogPost.created_at)
      .split('.')
      .join('.  '),
