...
const handler = async (
  ...
  const { search: searchParam, pageSize: pageSizeParam, page: pageParam } = req.query
  ...
  try {
    result = shouldMockGinis() ? mockedParsedDocuments : await getOfficialBoardParsedList(search)
  } catch (error) {
    // TODO handle error
    console.log(error)
  }
