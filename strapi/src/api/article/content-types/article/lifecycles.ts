...
//  * - we want to connect to a specific slug value, and also to an admin role
...
//
// const getAdminGroup = async ({ slug }: { slug: AdminGroupSlug }) => {
//   try {
...
//       throw new Error(`found no adminGroups for slug='${slug}'`)
//
//     // slug has unique values, so we get at most one result
...
//
//     const starzAdminGroup = await getAdminGroup({ slug: 'starz' })
//
